...
bool generateExport(
  ...
  final File exportFile = File(
      '${directory.absolute.path}/$directoryName$ext.dart');
  ...
  for (var item in items.where((element) => element is Directory)) {
    if (item is Directory && recursive) {
      ...
      exports +=
          "export '$subDirName/$subDirName.exports.dart';\n";
      continue;
    }
  }
...
void fixImport(File file) {
  ...
  for (final line in lines) {
    if (!line.trim().startsWith('//') && line.contains('package:creator')) {
      hasImport = true;
    } else {
      content += line + '\n';
    }
  ...
  if (hasImport) {
    content = "import 'package:creator/creator.dart';\n$content";
  }
